...
try:
    ...
    HAS_APEX = False
    print('apex is not installed, using PyTorch native AMP')
import mmcv
import torch
...
class ValBestPathHook(Hook):
    ...
    def __init__(self,
        ...
        self.madd_target = float(madd_target)
        self.madd_lambda = float(madd_lambda)
        self.madd_beta = float(madd_beta)
        if resume_best_path:
    ...
    @staticmethod
    def _soft_margin_penalty(madds, c_target, beta):
        excess = (madds - c_target) / c_target
        return max(0.0, excess) ** beta
    ...
    def _stage_block_madds(stage_idx, stage_path):
        ...
        base = block_base.get(stage_idx, block_base[3])
        total = 0.0
        for layer_idx, op_id in enumerate(stage_path):
            if layer_idx >= len(base):
                break
            total += base[layer_idx] * op_factor.get(int(op_id), 1.0)
        return total

    def _estimate_path_madds(self, model, stage_path):
        stem_fixed = 0.55e9
        total = stem_fixed
        for i, best_path in enumerate(model.best_paths):
            total += self._stage_block_madds(i, best_path)
        total += self._stage_block_madds(model.start_block, stage_path)
        return float(total)
        ...
        if self.every_n_epochs(runner, block_inteval) and model.start_block < model.num_block - 1:
            ...
            if model.use_fp16 and HAS_APEX:
                model, new_optimizer = apex.amp.initialize(model, new_optimizer,
                                                       opt_level="O1")
                print_log('**** Initializing mixed precision with Apex done. ****')
            elif model.use_fp16:
                print_log('**** Using PyTorch native AMP. ****')
    ...
    def multi_gpu_test(self, runner, data_loader):
        ...
        for op, online_result1 in path_online_results_v1.items():
            ...
            for i, proj_onlinev1 in enumerate(online_result1):
                ...
                world_size = dist.get_world_size() if is_dist_initialized() else 1
                loss_dict[op] += reduce_tensor(loss.data, world_size).item()
            loss_dict[op] /= len(online_result1)

            if self.use_soft_margin:
                stage_path = [int(x) for x in list(op)]
                madds = self._estimate_path_madds(model, stage_path)
                penalty = self._soft_margin_penalty(
                    madds,
                    self.madd_target,
                    self.madd_beta,
                )
                loss_dict[op] += self.madd_lambda * penalty
...
def update_bn_stats(bn_data, runner, forward_singleop):
    ...
    torch.cuda.synchronize()
    # Only distribute BN stats if distributed training is initialized
    if is_dist_initialized():
